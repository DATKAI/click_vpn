...
def _resolve(token: str):
    """Возвращает (status, meta, dir). status: ok|notfound|expired|exhausted."""
...
def download(token: str):
    ...
    if status == "notfound":
        return _page("Ссылка недействительна", "Возможно, она удалена.", 404)
    if status == "expired":
        return _page("Срок ссылки истёк", "Запросите новую ссылку.", 410)
    if status == "exhausted":
        return _page("Лимит скачиваний исчерпан", "Запросите новую ссылку.", 410)
